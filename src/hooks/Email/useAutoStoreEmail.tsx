import { approveEmail, CreateEmailParam, sendEmail, updateEmailWithQuery } from "@api/email";
...
const useAutoStoreEmail = (delayTime: number = 5000) => {
  ...
  const { mutate: storeDraftMutate, isLoading: isStoreDraftLoading } = useMutation({
    ...
    mutationFn: async (params: {data: CreateEmailParam, emailId?:number}) => {
      ...
      const draft = await approveEmail({ email_id: res.data.id, status: 'DRAFT', send_after: 0 })
      return draft;
    }
  });
  ...
  const onFieldsChange = (name: string | undefined, values: Partial<EmailComposeFields>) => {
  ...
  useEffect(() => {
    ...
    const emailData =  {
      ...
      to: workingEmail.to?.map(value => value.mail) ?? [],
      html_string: contentString,
      bcc: workingEmail.bcc?.map(value => value.mail) ?? [],
      cc: workingEmail.cc?.map(value => value.mail) ?? [],
      files: (workingEmail.attachFiles?.fileUrls.filter(value => value !== undefined) as string[])?.map(value => ({path: value})) ?? [],
