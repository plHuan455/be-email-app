        ...
        dispatch(
          setNotificationList(
            res.data.map((value) => ({
              id: value.id,
              body:
                value.notify_type_id === 1
                  ? `${value.sender} has sent a new email to ${value.receiver}. `
                  : '',
          ...
          {notificationList.map((notify) => (
            <Box
              ...
              <Box sx={{ minWidth: rem(12) }}>
                {!Boolean(notify.isSeen) && (
                  <FiberManualRecordIcon
                    sx={{ color: '#2172f2', fontSize: rem(12) }}
                  />
                )}
              </Box>
              <Box
