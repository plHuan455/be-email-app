...
import AutoCompleteGroup, { AutoCompleteGroupValueTypes } from '@components/molecules/AutoCompleteGroup';
...
interface EmailComposeProps {
  ...
  onDepartmentClick?: (option: AutoCompleteGroupValueTypes, field: EmailComposeEmailFieldNames) => void;
  onSelectEmployersChange?: (emails: string[]) => void;
  onCloseSelectEmployersModal?: () => void;
  onConfirmSelectEmployersModalClick?: () => void;
  onSelectedDepartmentClick?: (option: AutoCompleteGroupValueTypes, field: EmailComposeEmailFieldNames) => void;
  onDeleteDepartmentOnInput?: (option: AutoCompleteGroupValueTypes, field: EmailComposeEmailFieldNames) => void;
}
    ...
    <Box className="t-emailCompose w-full h-full py-10 mt-4">
      <FormProvider {...method}>
        <form className="p-8 flex items-center justify-center w-full h-full">
          <Box
            className={`flex flex-col h-full w-full mx-auto shadow-xl bg-white rounded-3xl overflow-hidden z-[80] transition-all ${isFullScreen && 'fixed top-0 left-0 bottom-0'
              }`}>
            ...
            <Box
              ...
              <Box className="px-9 py-10 pt-2 flex-1 flex flex-col">
                ...
                >
                    ...
                    render={({ field: { value, onChange } }) => (
                      <Box sx={{ flexGrow: 1 }}>
                        <AutoCompleteGroup
                          ...
                          onChange={onChange}
                          onChipClick={(option) => onSelectedDepartmentClick && onSelectedDepartmentClick(option, 'to')}
                          onChipDelete={(option) => onDeleteDepartmentOnInput && option.isGroup && onDeleteDepartmentOnInput(option, 'to')}
                        />
                      </Box>
                    )}
                  />
                  ...
                  >
                    <Box sx={{ width: '80vw' }}>
                      <Typography sx={{ py: rem(4) }}>{selectedDepartment?.data.name}</Typography>
                      <DataGrid
                        sx={{ height: '50vh' }}
                        ...
                        onSelectionModelChange={(emails) => { onSelectEmployersChange && onSelectEmployersChange(emails as string[]) }}
                      />
                      <Box display="flex" justifyContent="flex-end" sx={{ mt: rem(8) }}>
                        <Button
                          sx={{ backgroundColor: '#dc3545', '&:hover': { backgroundColor: '#bf192a' } }}
                          onClick={onCloseSelectEmployersModal}
                        >
                          Cancel
                        </Button>
                        <Button sx={{ ml: rem(20) }} onClick={onConfirmSelectEmployersModalClick}>
                          OK
                        </Button>
                      </Box>
                    </Box>
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Box sx={{ flexGrow: 1 }}>
                            <AutoCompleteGroup
                              ...
                              onGroupClick={(e, option) => {
                                if (option.id && option.isGroup) {
                                  onDepartmentClick && onDepartmentClick(option, 'cc');
                                }
                              }}
                              onChange={onChange}
                              onChipClick={(option) => onSelectedDepartmentClick && onSelectedDepartmentClick(option, 'cc')}
                              onChipDelete={(option) => onDeleteDepartmentOnInput && option.isGroup && onDeleteDepartmentOnInput(option, 'cc')}
                            />
                          </Box>
                        )}
                      />
                        ...
                        render={({ field: { value, onChange } }) => (
                          <Box sx={{ flexGrow: 1 }}>
                            <AutoCompleteGroup
                              ...
                              onGroupClick={(e, option) => {
                                if (option.id && option.isGroup) {
                                  onDepartmentClick && onDepartmentClick(option, 'bcc');
                                }
                              }}
                              onChange={onChange}
                              onChipClick={(option) => onSelectedDepartmentClick && onSelectedDepartmentClick(option, 'bcc')}
                              onChipDelete={(option) => onDeleteDepartmentOnInput && option.isGroup && onDeleteDepartmentOnInput(option, 'bcc')}
                            />
                          </Box>
                        )}
                      />
