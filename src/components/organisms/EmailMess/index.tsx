...
const EmailMess: React.FC<Props> = ({
  ...
  const _renderActionsApproved = useCallback(({ remainMinute }: { remainMinute: number, onCancel: () => void }) => {
    return (
              ...
              <Box>
                {sentAt.getTime() > Date.now() &&
                  _renderActionsApproved({
                    remainMinute: Math.round((sentAt.getTime() - (new Date().getTime())) / 1000 / 60),
                    onCancel: () => { }
                  })}
              </Box>
              <Box>
